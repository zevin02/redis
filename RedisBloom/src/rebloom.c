...
static size_t BFDefaultInitCapacity = 100;//默认的容量
...
typedef enum { SB_OK = 0, SB_MISSING, SB_EMPTY, SB_MISMATCH } lookupStatus;//查询状态

typedef struct {
    long long capacity;//用于指定初始化的容量
    double error_rate;//布隆过滤器的错误率
    int autocreate;//当想不存在的bloom filter中添加元素的时候，是否需要自动创建一个布隆过滤器
    // int must_exist;
    int is_multi;//是否一次性添加多个元素
    long long expansion;//容量满的时候，需要扩展的元素大小
    long long nonScaling;//容量耗尽的时候是否停止向其添加新元素
} BFInsertOptions;//插入时候的选项
...
static int getValue(RedisModuleKey *key, RedisModuleType *expType, void **sbout) {
    *sbout = NULL;
    ...
    if (type == REDISMODULE_KEYTYPE_EMPTY) {
        ...
        *sbout = RedisModule_ModuleTypeGetValue(key);//根据key获得相应的value
        return SB_OK;
    } else {
        return SB_MISMATCH;
    }
...
static SBChain *bfCreateChain(RedisModuleKey *key, double error_rate, size_t capacity,
    ...
    SBChain *sb = SB_NewChain(capacity, error_rate, BLOOM_OPT_FORCE64 | scaling | BLOOM_OPT_NOROUND,
                              expansion);//不进位,不会向最接近的2的倍数来转化
    if (sb != NULL) {
        RedisModule_ModuleTypeSetValue(key, BFType, sb);//把redisbloom中的key关联到相应的SBChain
    }
    return sb;
}
...
//keystr就是要操作的bf的key，items就是要插入的元素，nitems就是要插入的元素个数
static int bfInsertCommon(RedisModuleCtx *ctx, RedisModuleString *keystr, RedisModuleString **items,
    ...
    SBChain *sb;
    const int status = bfGetChain(key, &sb);//先获得对应的chain

    if (status == SB_EMPTY && options->autocreate) {//如果为空，通社设置了自动创建，我们就需要创建
        sb = bfCreateChain(key, options->error_rate, options->capacity, options->expansion,
                           options->nonScaling);//创建一个新的sbchain
        if (sb == NULL) {
    ...
    if (options->is_multi) {//如果这个地方是添加多个，就走这里，
        RedisModule_ReplyWithArray(ctx, REDISMODULE_POSTPONED_ARRAY_LEN);
    }

    size_t array_len = 0;//数组长度
    int rv = 0;
    for (size_t ii = 0; ii < nitems && rv != -2; ++ii) {//这个地方遍历插入的items
        size_t n;
        const char *s = RedisModule_StringPtrLen(items[ii], &n);//这里将items[ii]转化成了指针
        rv = SBChain_Add(sb, s, n);//把元素s插入进去
        if (rv == -2) { // decide if to make into an error
            RedisModule_ReplyWithError(ctx, "ERR non scaling filter is full");
        } else {
            RedisModule_ReplyWithLongLong(ctx, !!rv);//两个！可以把一个变量转化成布尔值1->true,0->false
        }
        array_len++;//插入成功，数组长度+1
    }

    if (options->is_multi) {
        RedisModule_ReplySetArrayLength(ctx, array_len);
    }
    RedisModule_ReplicateVerbatim(ctx);//将操作复制到对应的slave和aof上
    return REDISMODULE_OK;
}
 ...
 * dump是为了序列化给定的key，返回一个二进制流，可以将redis的某个key转化成纯文本进行备份和持久化，和restore命令一起使用来进行redis备份数据的迁移(可以用来主从复制，和集群数据迁移，先进行序列化之后再发送给给定的节点)
 通过这个命令加载导出的文件，并将元素添加到一个新的布隆过滤器中
 127.0.0.1:6379> BF.SCANDUMP my_filter 0
1) (integer) 0
2) "\x00\x00\x00\x00\x00"          从0位置开始，迭代器从0开始进行压缩，返回其二进制的形式
 */
static int BFScanDump_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    const SBChain *sb = NULL;
    RedisModuleKey *key = RedisModule_OpenKey(ctx, argv[1], REDISMODULE_READ);
    int status = bfGetChain(key, (SBChain **)&sb);//获得对应的布隆过滤器
    if (status != SB_OK) {
        return RedisModule_ReplyWithError(ctx, statusStrerror(status));
    }

    long long iter;
    //把第二个元素的迭代的索引位置设置到iter中
    ...
    if (iter == 0) {//这个就是从头开始进行一个迭代
        size_t hdrlen;
        //from head .so included the whole bloom 
        char *hdr = SBChain_GetEncodedHeader(sb, &hdrlen);
        RedisModule_ReplyWithLongLong(ctx, SB_CHUNKITER_INIT);
        RedisModule_ReplyWithStringBuffer(ctx, (const char *)hdr, hdrlen);//send the encoded string back
        SB_FreeEncodedHeader(hdr);//free the hdr pointer
    } else {
        size_t bufLen = 0;
        const char *buf = SBChain_GetEncodedChunk(sb, &iter, &bufLen, MAX_SCANDUMP_SIZE);//获得最大的元素
 ...
 * 对应scandump，将序列化之后的bf加载这里
 * 127.0.0.1:6379> BF.LOADCHUNK new_filter 0 "\x00\x00\x00\x00\x00"
(integer) 1
将我们上面的二进制的数据进行添加到一个新布隆过滤器中
 */
static int BFLoadChunk_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    size_t bufLen;
    const char *buf = RedisModule_StringPtrLen(argv[3], &bufLen);//这个里面就是放到压缩后的数据，buf就是构造好的dump结构体

    RedisModuleKey *key = RedisModule_OpenKey(ctx, argv[1], REDISMODULE_READ | REDISMODULE_WRITE);
    SBChain *sb;
    int status = bfGetChain(key, &sb);//先尝试是否能够在本地获得到对应的key，如果能获得到的话，就在本地操作
    if (status == SB_EMPTY && iter == 1) {//没有数据，或者
        const char *errmsg;
        SBChain *sb = SB_NewChainFromHeader(buf, bufLen, &errmsg);//将dump里面的数据提取，并放到sb中
        if (!sb) {
...
typedef struct {
    int is_nx;//判断是否是addnx命令
    int autocreate;
    int is_multi;
    long long capacity;//容量,基类的节点最小能存储的元素个数
} CFInsertOptions;
...
static int cfInsertCommon(RedisModuleCtx *ctx, RedisModuleString *keystr, RedisModuleString **items,
    ...
    int status = cfGetFilter(key, &cf);//根据key获得cf
    ...
    if (options->is_multi) {//判断是否需要添加多个元素
        RedisModule_ReplyWithArray(ctx, nitems);
    }

    for (size_t ii = 0; ii < nitems; ++ii) {
        size_t elemlen;
        const char *elem = RedisModule_StringPtrLen(items[ii], &elemlen);//获得item
        CuckooHash hash = CUCKOO_GEN_HASH(elem, elemlen);//根据item获得哈希值
        CuckooInsertStatus insStatus;
        if (options->is_nx) {
            insStatus = CuckooFilter_InsertUnique(cf, hash);
        } else {
            insStatus = CuckooFilter_Insert(cf, hash);//调用插入
        }
        switch (insStatus) {
        case CuckooInsert_Inserted:
            RedisModule_ReplyWithLongLong(ctx, 1);//插入成功,回复1
            break;
...
static int CFCheck_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    RedisModuleKey *key = RedisModule_OpenKey(ctx, argv[1], REDISMODULE_READ);
    CuckooFilter *cf;
    int status = cfGetFilter(key, &cf);//根据key获得相应的CuckooFilter
    ...
    for (size_t ii = 2; ii < argc; ++ii) {
        if (is_empty == 1) {
            ...
            long long rv;
            if (is_count) {
                rv = CuckooFilter_Count(cf, hash);//统计hash出现的次数，全局进行操作
            } else {
                rv = CuckooFilter_Check(cf, hash);//全局查看hash是否出现
            }
...
static int BFCard_RedisCommand(RedisModuleCtx *ctx, RedisModuleString **argv, int argc) {
    ...
    RedisModule_ReplyWithLongLong(ctx, bf->size);//通过他的size进行返回
...
static size_t BFMemUsage(const void *value) {
    ...
    for (size_t ii = 0; ii < sb->nfilters; ++ii) {
        ...
        rv += sb->filters[ii].inner.bytes;//把所有的bytes加和即可
    }
    return rv;
}
